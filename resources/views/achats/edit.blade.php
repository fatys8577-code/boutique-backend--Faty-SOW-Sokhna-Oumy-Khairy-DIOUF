    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('achats.update', $achat) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="produit_id" class="mt-1 block w-full rounded-md border-gray-300">
                            ...
                            @foreach ($produits as $produit)
                                <option value="{{ $produit->id }}" {{ old('produit_id', $achat->produit_id) == $produit->id ? 'selected' : '' }}>
                                    {{ $produit->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="acheteur_id" class="mt-1 block w-full rounded-md border-gray-300">
                            ...
                            @foreach ($acheteurs as $acheteur)
                                <option value="{{ $acheteur->id }}" {{ old('acheteur_id', $achat->acheteur_id) == $acheteur->id ? 'selected' : '' }}>
                                    {{ $acheteur->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="quantite" min="1" value="{{ old('quantite', $achat->quantite) }}" class="mt-1 block w-full rounded-md border-gray-300">
                        @error('quantite')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_achat" value="{{ old('date_achat', $achat->date_achat->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300">
                        @error('date_achat')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
